...
## 2026-09-06 · Tre chain diverse, BSC abbandonata — e il consulente esterno entra in pianta stabile
- **BSC fuori:** peggiore su ogni misura (-21% media, -24% robusta, 14% di trade in guadagno) e la
  piu' povera di token giudicabili. Non ha senso spenderci banda.
- **Restano Base, Robinhood e Solana** — non per prudenza, ma perche' sono **tre mercati diversi**:
  microstrutture, ecosistemi e tipi di token diversi. **Lo strumento che cerchiamo potrebbe esistere
  in uno solo dei tre**, e tre possibilita' vere valgono piu' di quattro mezze.
- **Astra passa a DUE consulenze al giorno**, e nel prompt ora c'e' scritto come funziona il rapporto:
  *quello che proponi non finisce in un documento, viene costruito entro poche ore*. Cambia cosa gli
  conviene scrivere — puo' proporre cose grosse invece di trattenersi, e puo' dirci **cosa costruire**,
  non solo cosa correggere.
- **Bersaglio dichiarato: +8% netto per operazione.** Oggi la chain migliore fa +4% di media e +1%
  robusta. Serve **raddoppiare, non limare**.
- **Quanto capitale investire si decide DOPO.** Deciderlo prima ci ha fatto perdere un'ora su quanti
  trade servono per arrivare a 3.000 euro — domanda che non ha senso finche' la percentuale non c'e'.
- **La nostra forza sono l'architettura e il tempo:** una macchina che gira sola a costo zero, un
  assistente che costruisce in fretta, e un consulente esterno che vede la cornice. Non serve avere
  ragione oggi: serve poter provare molte idee e ucciderne in fretta quelle sbagliate.
- **Esito:** in corso.

## 2026-09-06 · LOOP 1 ottimizza la CRESCITA DEL CAPITALE, non piu' la robusta
- **Il goal dichiarato da Nicolo':** partire da **100 euro**, arrivare a **3.000** con guadagni piccoli
  **reinvestiti**, e da li' mettere via 500 euro. Non e' un dettaglio di scala: cambia la matematica.
- **L'errore:** ottimizzavamo la media aritmetica dei rendimenti, e la ROBUSTA (che butta via il 5%
  dei risultati migliori). Due difetti gravi per questo obiettivo:
  1. **La media mente quando reinvesti.** +50% e poi -50%: la media dice zero, il capitale dice -25%.
     Reinvestendo una perdita pesa piu' di un guadagno uguale, e la media non lo vede.
  2. **La robusta scartava proprio le strategie che fanno crescere il capitale.** Serviva a non farsi
     ingannare da tre colpi fortunati, ma cercava strategie che funzionano ANCHE SENZA i colpi
     grossi — cioe' quelle che a 3.000 non arrivano mai.
- **Decisione:** il punteggio diventa **quanto diventano 100 euro reinvestendo**, in ordine di tempo,
  con una quota per posizione (20%) invece del tutto-dentro. La **robusta resta come stress test**:
  stesso trattamento dato al costo del 33% — il numero severo non sparisce, smette di decidere.
- **Il metro non si tocca.** Costi misurati, trappole, ritardo reale, niente look-ahead, controlli
  appaiati: quelli valgono a qualunque capitale. *Misurare bene* e *cosa premiare* sono due cose
  diverse, e le avevo trattate insieme.
- **Numero onesto emerso subito:** con +8% netto a trade e il 20% del capitale per posizione servono
  **214 trade** per andare da 100 a 3.000. A 2-3 trade al giorno sono circa due mesi, non uno.
- **Dove va cosa:** questo cambia LOOP 1 (cosa premia). Le idee fuori dal database — eventi
  verificati incrociati con flussi obbligati — vanno nel LOOP SPERIMENTALE, separato.
- **Esito:** in corso.

## 2026-09-06 · La banda va dove ci sono le prove: Robinhood e Solana in primo piano
- **Il conto che ha deciso:** servono **25 prove indipendenti** (giornate distinte, perche' cio' che
  nasce lo stesso giorno si muove insieme). Stato al 06/09:

  | chain | token valutabili | giornate | pronta il |
  |---|---|---|---|
  | robinhood | **46** | 5 | **26 settembre** |
  | solana | **33** | 6 | **25 settembre** |
  | base | 7 | 2 | 29 settembre |
  | bsc | 6 | 2 | 29 settembre |

- **Il dato che pesa:** su Base ci sono **1.463 token nati dopo il sigillo e SETTE giudicabili**.
  Stavamo dando la fetta di banda piu' grande alla chain con la resa piu' bassa.
- **Perche' concentrare:** le prove NON si sommano fra chain. Sei giornate su Solana piu' sei su BSC
  non fanno dodici: sono mercati diversi, e serve arrivare a 25 su CIASCUNA. Dividere in quattro parti
  uguali significa averne bisogno di cento invece di venticinque.
- **Perche' DUE chain e non una:** un segnale che regge su Solana **e** su Robinhood — microstruttura
  diversa, ecosistemi diversi — e' evidenza molto piu' forte di uno che regge su una sola. Non e'
  prudenza: e' un test piu' severo, e costa poco perche' entrambe sono gia' vicine alla maturita'.
- **Cosa NON facciamo:** Base e BSC non si spengono. Restano in sottofondo e se maturano le testiamo.
  Riduciamo la priorita', non chiudiamo la porta.
- **La scadenza resta il 3 ottobre.** Il conto dice che ci arriviamo con una settimana di margine:
  spostarla sarebbe cedere senza motivo. Il numero che manca sono le GIORNATE, e quelle arrivano da
  sole — una al giorno, qualunque cosa facciamo.
- **Esito:** in corso.
...
## 2026-09-21 — Solana resta ferma, e adesso è ferma davvero

**Decisione del fondatore:** «Solana ferma».

**Cosa cambia in concreto.** Solana era dichiarata fuori dal perimetro dal 19/09, ma non lo era:
`solana_helius` era ancora attiva e aveva raccolto un'ora prima di questa verifica. Occupava posti
nella coda a venti lavori di GitHub — cioè rallentava la riparazione di robinhood, che è la misura
che tiene fermo il loop 1. Ora è `disabled_manually`, come le altre cinque corsie che la nominano.

**Cosa NON è stato toccato.** I 63 MB di dati Solana già raccolti restano intatti su disco. È
stata spenta la raccolta, non cancellato niente: se un giorno Solana rientra, si riparte da lì e
non da zero.

**Perché non adesso.** Solana è probabilmente la chain dove l'edge esiste davvero — è quella con
più movimento sulle memecoin. Ma non è mai stata costruita: candele ferme da 44 ore, censimento
assente, coppie assenti. Aprire un terzo cantiere mentre i primi due non sono chiusi significa
avere tre database a metà invece di uno buono — che è, con un nome diverso, lo stesso errore che
due mesi fa è costato un mese e mezzo di analisi su dati inaffidabili.

**Quando si riapre.** Dopo che base e robinhood hanno passato l'esame di accettazione e il loop 1
è ripartito. Se il loop 1 su due chain non trova nulla, il problema è il metodo e non i dati:
meglio scoprirlo su due chain che su tre.

## 2026-09-21 — L'osservatorio distingue lo spento dal rotto

**Il problema.** Il loop 0 dichiarava «36 COMPONENTI NON RISPONDONO — la macchina non è pronta».
Ma erano agenti spenti da noi (i ruoli del loop 1, fermi dal 17/09) su chain fuori perimetro
(solana e bsc, fuori dal 21/09), più una trentina di componenti del vecchio impianto che non
giriamo da quando tutto è concentrato sul database.

Nessuno di quelli era un guasto. Era una nostra scelta, **mai dichiarata da nessuna parte** — e una
scelta non dichiarata produce allarmi che nessuno può distinguere da quelli veri.

**Perché conta.** Un cruscotto che segnala rosso quando tutto va bene smette di essere letto, e
quando il rosso sarà vero non se ne accorgerà nessuno. È lo stesso difetto che la revisione esterna
ha trovato in `STAFFETTA.md`.

**La decisione.** Tre stati invece di due: *funziona*, *fermo per decisione*, *rotto*. Solo il terzo
è un allarme. Il secondo non è silenzio: si mostra con la data, così «spento e dimenticato» resta
visibile senza gridare.

Restano allarme vero solo i componenti che proteggono a prescindere dalla fase: **CFO e Tassametro**
(i soldi), **Security e Censore** (la sicurezza), **Heartbeat** (il battito), **Anagrafe, Memoria e
Verità** (ciò che tiene in piedi il database). Tutti gli altri appartengono all'impianto che la
nuova metodologia sostituisce.

**Reversibile:** quando il loop 1 si accende, i suoi ruoli tornano automaticamente sotto allarme.

## 2026-09-22 — I timbri di verifica anteriori alle 02:35 non valgono su base, valgono su robinhood

**Il fatto.** Fino alle 02:35 UTC del 22/09 il riparatore abbinava le risposte del nodo ai blocchi
per **posizione nella richiesta** invece che per **numero di blocco contenuto nella risposta**. Il
nodo pubblico di base tronca i gruppi (misurato: 10 blocchi ok, 15 troncati), quindi le posizioni
saltavano e ogni blocco riceveva l'orario di un altro — con errori da 18 minuti a 2,4 ore. E il
record risultava «verificato», perché formalmente l'orario era stato chiesto.

**La decisione, e perché non è uguale sulle due chain.** I timbri messi prima di quel momento sono
invalidati **su base** e **tenuti su robinhood**. Non è una scorciatoia: è una misura.

| | controllo indipendente contro la catena, solo timbri vecchi |
|---|---|
| base | **1 record sbagliato su 45** (errore di 3.176 secondi) |
| robinhood | **120 su 120 corretti**, nessun errore |

Il nodo di robinhood non tronca, quindi l'abbinamento per posizione dava comunque il risultato
giusto. Invalidare anche lì costerebbe ore di rilavorazione per correggere un errore che le misure
dicono non esistere.

**Cosa la farebbe cambiare:** un solo controesempio su robinhood — un record con timbro anteriore
alle 02:35 e orario sbagliato. Se emerge, si invalida anche lì.

**Costo della decisione:** base torna dal 41,8% al 28,8% di dati dimostrabilmente verificati. È un
passo indietro nei numeri e un passo avanti nella verità.
