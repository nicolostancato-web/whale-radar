...
def main():
    ...
    # SI UNISCE CON QUELLO CHE C'E' GIA' (20/09). Qui si riscriveva l'INTERO dizionario delle
    # nascite: due corse sovrapposte della stessa corsia — che capitano — si cancellavano a vicenda
    # il lavoro, perche' con «-X ours» vince l'ultima che spinge, anche se ne sapeva meno.
    # Trovato sul segnalibro della riparazione, che era tornato indietro da 9.153 pool a 2.000.
    # Fra due versioni della stessa nascita vince quella RISOLTA DALLA CATENA: una nascita dedotta
    # dalle candele non deve mai sovrascriverne una verificata.
    try:
        _vecchie = json.load(open(OUT)).get("nascite", {}) or {}
    except Exception:
        _vecchie = {}
    _recuperate = 0
    for _p, _v in _vecchie.items():
        _mia = note.get(_p)
        if _mia is None:
            note[_p] = _v
            _recuperate += 1
        elif _v.get("fonte") == "catena" and _mia.get("fonte") != "catena":
            note[_p] = _v
            _recuperate += 1
    if _recuperate:
        print(f"NASCITA | {CHAIN}: {_recuperate} nascite recuperate da un'altra corsa", flush=True)
