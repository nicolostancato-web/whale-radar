...
def matched_su_momentum(U, addr, ts, k=5, candidati=None, tolleranza=0.5):
    """Controlli appaiati ANCHE sul rialzo gia' avvenuto (aggiunto 06/09, dalla revisione esterna).

    Fin qui il placebo all'indietro l'ho usato come ghigliottina: "prima del segnale il token aveva
    gia' fatto +307%, quindi e' momentum, quindi e' morto". La critica e' che quel ragionamento non
    dimostra cio' che credevo:

        «Un segnale puo' descrivere il passato E prevedere il futuro: basta che il fenomeno persista.
         "Prima era gia' salito del 307%" dimostra che arrivate dopo un rialzo; non dimostra che
         arrivate troppo tardi.»

    Ha ragione. La domanda giusta non e' "c'era momentum?" ma: *a parita' di rialzo gia' avvenuto,
    quel segnale aggiunge qualcosa?* Se fra due token che hanno fatto entrambi +300% quello segnalato
    rende di piu', il segnale vale — anche se descrive il passato.
    Potrei aver bocciato un candidato vivo. Qui si costruisce il confronto onesto: controlli con lo
    STESSO rialzo precedente, oltre che stessa eta' e liquidita'."""
    cs = U.cs.get(addr)
    if not cs: return []
    mio = placebo(cs, ts, [7200]).get(7200)
    if mio is None: return U.matched(addr, ts, k, candidati)
    base = U.matched(addr, ts, k * 6, candidati)
    simili = []
    for a in base:
        suo = placebo(U.cs[a], ts, [7200]).get(7200)
        if suo is None: continue
        if abs(suo - mio) <= max(tolleranza, abs(mio) * tolleranza): simili.append(a)
        if len(simili) >= k: break
    # se non ce ne sono abbastanza col rialzo simile, meglio pochi confronti onesti che tanti sbagliati
    return simili if len(simili) >= 3 else []
