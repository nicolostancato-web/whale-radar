...
def catena(chain, pool, dato):
    """Una domanda sola al nodo non basta (18/09): il nodo pubblico di base risponde "0x" a una
    eth_call valida quando e' sotto carico, senza segnalare errore. Qui non produceva una risposta
    sbagliata — il chiamante conclude "non so" — ma faceva perdere pool VIVI classificandoli come
    ignoti. Le riprove stanno in chiamata.eth_call, che distingue il nodo che tace dal contratto
    che nega."""
    v, _stato = C.eth_call(RPC[chain], pool, dato, tentativi=3, pausa=0.6, timeout=20)
    return v
