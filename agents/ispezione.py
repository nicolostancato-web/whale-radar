...
# TRE STATI, NON DUE (21/09). L'osservatorio dichiarava «36 COMPONENTI NON RISPONDONO — la
# macchina non e' pronta», elencando Esploratore, Ricercatore e Cervello su quattro chain. Ma
# quegli agenti li abbiamo spenti NOI il 17/09, e due di quelle chain sono fuori perimetro dal
# 21/09. L'osservatorio stava segnalando come guasto una nostra decisione.
# Il danno non e' il falso allarme: e' che un cruscotto che segnala rosso quando tutto va bene
# SMETTE DI ESSERE LETTO, e quando il rosso sara' vero non se ne accorgera' nessuno.
# Gli stati sono: funziona / fermo per decisione / rotto. Solo il terzo e' un allarme. E il
# secondo non e' silenzio: si mostra, cosi' «spento e dimenticato» resta visibile senza gridare.
FUORI_PERIMETRO = {"solana", "bsc"}          # decisione del 21/09, in DECISIONS.md
LOOP1_SPENTO_DAL = "17/09"                   # le corsie sono disabled_manually, verificato
RUOLI_DEL_LOOP1 = ("splorat", "icercator", "ervell")   # Esploratore, Ricercatore, Cervello

# CHI DEVE LAVORARE DURANTE LA FASE DATABASE, E CHI NO (21/09).
# Trenta componenti risultavano guasti perche' non girano da quando tutto e' concentrato sul
# database. Non erano rotti: era una nostra scelta, mai dichiarata da nessuna parte. Una scelta non
# dichiarata produce allarmi che nessuno puo' distinguere da quelli veri.
#
# Restano allarme VERO solo quelli che proteggono a prescindere dalla fase: i soldi (CFO,
# Tassametro), la sicurezza (Security, Censore), il battito (Heartbeat) e cio' che tiene in piedi
# il database stesso (Anagrafe, Memoria, Verita').
# Tutti gli altri appartengono all'impianto che la nuova metodologia sostituisce: si mostrano come
# fermi, con la data, cosi' «spento e dimenticato» resta visibile senza gridare.
SEMPRE_ATTIVI = {"CFO", "Tassametro", "Security", "Censore", "Heartbeat",
                 "Anagrafe", "Memoria", "Verità"}
FASE_DATABASE_DAL = "17/09"
...
def interroga():
    ...
    for nome, compito, file_t, minuti, chain in TEAM:
        # le chain fuori perimetro non si elencano nemmeno: non sono ferme, non ci sono
        if chain in FUORI_PERIMETRO:
            continue
        # i ruoli del loop 1 sono fermi per decisione finche' il loop 1 e' spento
        if any(k in nome for k in RUOLI_DEL_LOOP1):
            esiti.append((nome, compito, chain, None, minuti,
                          f"⏸️ fermo per decisione — loop 1 spento dal {LOOP1_SPENTO_DAL}"))
            continue
        if nome not in SEMPRE_ATTIVI:
            esiti.append((nome, compito, chain, None, minuti,
                          f"⏸️ fermo per decisione — fase database dal {FASE_DATABASE_DAL}"))
            continue
...
CHAINS = ["base", "robinhood"]        # solana e bsc fuori perimetro dal 21/09
