...
# QUANTO TEMPO HO DAVVERO (08/09). Il workflow dedicato mi da' 20 minuti, ma il motore mi chiama con
# un tetto di 320 secondi: con 45 pool non ci sto dentro, e venivo ucciso a meta' strada. Fin qui il
# segnalibro si salvava SOLO alla fine, quindi ogni morte buttava via tutto il giro e il successivo
# ripartiva dagli stessi pool: da fuori sembrava lentezza, era lavoro rifatto ogni volta.
# Ora so quanto tempo ho e mi fermo PRIMA, salvando. Chi non sa quando finisce non finisce mai.
SECONDI = float(os.environ.get("CANDLE_SECONDI", 0)) or None
INIZIO = time.time()
    ...
    for i, (addr, name) in enumerate(todo):
        for tf in ("hour", "day"):
            ...
            for x in d.get("data", {}).get("attributes", {}).get("ohlcv_list", []):
                ...
                seen.add(sid); nc += 1
                fc.write(json.dumps({"acq": now, "pool": addr, "tf": tf, "ts": int(x[0]), "o": x[1], "h": x[2], "l": x[3], "cl": x[4], "v": round(x[5])}) + "\n")
        ck[addr] = now
        if (i + 1) % 5 == 0:
            json.dump(ck, open(CK, "w"))          # segnalibro incrementale: una morte non azzera il giro
        if (i + 1) % 20 == 0:
            print(f"  ...{i+1}/{len(todo)} pool, +{nc} candele", flush=True)
        if SECONDI and time.time() - INIZIO > SECONDI - 12:
            print(f"  mi fermo a {i+1}/{len(todo)}: finito il tempo, segnalibro salvato", flush=True)
            break
    fc.close()
    if nc == 0:
        os.remove(cf)
