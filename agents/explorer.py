...
# CAPITALE DA 100 EURO, PROFITTI REINVESTITI (06/09). Il goal di Nicolo': partire da 100 e arrivare a
# 3.000 con guadagni piccoli rimessi dentro. Cambia la MATEMATICA dell'obiettivo, non solo la scala.
#
# Finora ottimizzavamo la media aritmetica dei rendimenti, e con i profitti reinvestiti quella media
# MENTE. Due trade, +50% e -50%: la media dice zero, il capitale dice -25%. Reinvestendo, una perdita
# pesa piu' di un guadagno uguale, e la media aritmetica non lo vede.
# Peggio: ottimizzavamo la ROBUSTA, che butta via il 5% dei risultati migliori. Serviva a non farsi
# ingannare da tre colpi fortunati, ma penalizzava proprio le strategie che fanno crescere il
# capitale. Cercavamo strategie che funzionano ANCHE SENZA i colpi grossi, cioe' quelle che a 3.000
# non ci arrivano mai.
# Da qui si ottimizza quello che conta: quanto diventano 100 euro alla fine.
CAPITALE_INIZIALE = 100.0
OBIETTIVO = 3000.0
QUOTA = 0.20        # quanto del capitale in ogni posizione: 5 posizioni aperte al massimo


def crescita(rr, quota=QUOTA):
    """Quanto diventano 100 euro reinvestendo, in ordine di tempo. E' il numero che conta davvero.

    `quota` evita la finzione del tutto-dentro-ogni-volta: si rischia una frazione per posizione,
    quindi una perdita totale non azzera il conto e una vincita non moltiplica tutto.
    Restituisce la crescita in percentuale, perche' il resto del codice ragiona in percentuali."""
    if not rr: return 0.0
    cap = 1.0
    for x in rr:
        cap *= (1 + quota * max(-1.0, x))
        if cap <= 0.01: return -99.0        # conto bruciato: nessun ritorno da qui
    return (cap - 1) * 100


def quanti_trade_per_obiettivo(rr, quota=QUOTA):
    """Con questa strategia, quanti trade servono per andare da 100 a 3.000 euro?
    Se il capitale non cresce, non ci si arriva mai e lo diciamo invece di stimare un numero enorme."""
    if not rr: return None
    import math as _m
    cap = 1.0
    for x in rr: cap *= (1 + quota * max(-1.0, x))
    if cap <= 1.0 or len(rr) < 5: return None
    per_trade = cap ** (1.0 / len(rr))
    if per_trade <= 1.0: return None
    return int(_m.log(OBIETTIVO / CAPITALE_INIZIALE) / _m.log(per_trade))
...
def valuta(rows, mask, thr, dettaglio=False):
    ...
    # IL PUNTEGGIO E' LA CRESCITA DEL CAPITALE, non piu' la robusta (06/09).
    # La robusta resta calcolata e mostrata come STRESS TEST — stesso trattamento dato al costo del
    # 33%: il numero severo non sparisce, smette solo di decidere.
    # Si prende il minimo fra crescita totale e crescita sulla meta' recente: una strategia che
    # cresceva un mese fa e oggi e' morta non deve vincere.
    meta2 = len(sel) // 2
    cre_tot = crescita(sel)
    cre_rec = crescita(sel[meta2:]) if len(sel) >= 24 else cre_tot
    return min(cre_tot, cre_rec), porta(sel), len(sel), vinti, rob_tot, rob_rec, se, _limite_basso(sel), m
