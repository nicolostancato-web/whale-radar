...
# ROBINHOOD NON STA SU DEXSCREENER (10/09). Per questo il suo battito era a ZERO: liquidita', numero
# di compratori e venditori non li abbiamo mai raccolti, e meta' del laboratorio (gli esperimenti che
# guardano i soldi che entrano e chi vende) semplicemente non la vedeva. Una delle tre chain era cieca
# e il conteggio a zero non aveva mai fatto rumore.
# GeckoTerminal invece la copre, ed e' la stessa API gratuita da cui prendiamo gia' le candele.
GT = "https://api.geckoterminal.com/api/v2"
GT_CHAINS = {"robinhood": "robinhood"}
GT_BATCH = 25
...
def battito_gt(chain, rete, young, base, rubrica):
    """Il battito su GeckoTerminal: stesso formato di quello DexScreener, cosi' chi legge non cambia."""
    n = 0
    for i in range(0, len(young), GT_BATCH):
        chunk = young[i:i + GT_BATCH]
        d = get(f"{GT}/networks/{rete}/pools/multi/" + ",".join(chunk))
        time.sleep(PAUSE)
        for p in ((d or {}).get("data") or []):
            ...
            tk = (rel.get("id") or "").split("_")[-1]
            if tk: rubrica[addr.lower()] = tk
            ...
            try: vol = float((a.get("volume_usd") or {}).get("h1") or 0)
            except Exception: vol = 0.0
            tx = (a.get("transactions") or {}).get("h1") or {}
            row = {"acq": now, "ts": now, "op": price, "hi": price, "lo": price, "cl": price, "vol": vol,
                   "liq": round(liq, 2), "buys": tx.get("buys", 0), "sells": tx.get("sells", 0)}
            nato = a.get("pool_created_at")
            if nato:
                try:
                    row["t0"] = int(time.mktime(time.strptime(nato[:19], "%Y-%m-%dT%H:%M:%S")))
                except Exception: pass
            if tk: row["tk"] = tk
...
def main():
    tot = 0
    for chain, dsname in list(CHAINS.items()) + list(GT_CHAINS.items()):
        via_gt = chain in GT_CHAINS
        ...
        n = 0
        if via_gt:
            n = battito_gt(chain, dsname, young, base, rubrica)
            try: json.dump(rubrica, open(rf, "w"))
            except Exception: pass
            tot += n
            print(f"  {chain}: {n} punti su {len(young)} giovani (GeckoTerminal)", flush=True)
            continue
        for i in range(0, len(young), BATCH):
            ...
            for p in (d or {}).get("pairs") or []:
                ...
                # "acq" = quando il dato e' entrato DA NOI. Vedi multichain_trades: senza questo
                # timbro la domanda "ce l'avevamo al momento della decisione?" non ha risposta, e
                # ricostruirlo dalla storia git e' impossibile da quando il GC l'ha schiacciata.
                row = {"acq": now, "ts": now, "op": price, "hi": price, "lo": price, "cl": price, "vol": vol,
    ...
    try:
        ...
        _R.annota("pulse", "dexscreener+geckoterminal", "tutte", tot)
    except Exception:
        pass
