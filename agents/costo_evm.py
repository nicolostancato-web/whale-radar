...
"""
import json, os, sys, glob, gzip, time, urllib.request, statistics as st
...
def riserve(chain, pool):
    # UNA DOMANDA SOLA NON BASTA (18/09). Il nodo pubblico di base, sotto carico, risponde "0x" a
    # una eth_call valida senza segnalare errore. Qui il danno non era un costo sbagliato — il pool
    # veniva semplicemente saltato — ma un CAMPIONE DISTORTO: i costi finivano calcolati sui pool
    # che per caso avevano avuto una risposta, e la docstring qui sopra promette invece che chi non
    # risponde e' un pool stile V4. Con le riprove le due cose tornano a coincidere.
    res, stato = C.eth_call(RPC[chain], pool, "0x0902f1ac", tentativi=3, pausa=0.6, timeout=25)
    if stato != "ok" or not res or len(res) < 130: return None
    try:
...
def leggi(chain, pool, sel):
    """Stessa ragione di riserve(): un vuoto puo' essere il nodo che cede, non il contratto."""
    v, _stato = C.eth_call(RPC[chain], pool, sel, tentativi=3, pausa=0.6, timeout=25)
    return v
