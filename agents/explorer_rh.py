...
def main():
    # IL CONGELAMENTO PROTEGGE IL SIGILLO, NON FERMA LA RICERCA (corretto 06/09).
    # La prima versione bloccava l'intero esploratore: sbagliato, e me l'ha fatto notare Nicolo
    # chiedendo "perche' Robinhood e' stata interrotta?".
    # Le due cose non c'entrano niente l'una con l'altra: la ricerca gira SOLO sulla fascia di
    # ricerca (in_cassaforte esclude validazione e conferma), quindi non puo' contaminare la lettura
    # sigillata nemmeno volendo. Fermarla non proteggeva niente — toglieva soltanto una delle quattro
    # chain dal goal, che e' aumentare la percentuale su tutte.
    # Cio' che resta congelato e' la CONFIGURAZIONE registrata in criteri.json: quella non si tocca e
    # verra' giudicata una volta sola. Se la ricerca ne trova una migliore, sara' una candidata NUOVA,
    # da congelare a sua volta prima di essere giudicata.
    fermo, quando = _congelata()
    if fermo:
        print(f"EXPLORER_RH | c'e' una configurazione congelata dal {quando} in attesa del verdetto. "
              "La ricerca continua sulla sola fascia di ricerca: una candidata nuova non tocca il "
              "sigillo della vecchia.", flush=True)
    cand, flow, fbp, wl, fts = L.load_data()
    reg = json.load(open("data/pools.json"))["pools"] if os.path.exists("data/pools.json") else {}
    mp = {a: reg[a].get("name") for a in reg if len(a) == 42 and L._is_meme(reg[a].get("name"))}
    byname = {}
