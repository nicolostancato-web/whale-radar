...
def main():
    ...
    for chain, (url, ampiezza) in RPC.items():
        ...
        # SE IL NODO NON RISPONDE SI RIPROVA, E SE PROPRIO NO LO SI DICE (21/09).
        # Qui c'erano due «continue» muti: se il nodo rifiutava una di queste tre chiamate — e
        # robinhood rifiuta a intermittenza quando le corsie lo martellano — l'intera CHAIN veniva
        # saltata senza una riga di spiegazione.
        # Sintomo: l'audit produceva 33 finestre su base e ZERO su robinhood, e non «non
        # giudicabili»: proprio nessuna. Con 27.565 righe utili pronte per essere campionate.
        # E' la stessa forma di errore che inseguo da due giorni, stavolta nel controllo stesso:
        # un tentativo fallito che non lascia traccia e diventa «non c'era niente da fare».
        punta = None
        for _k in range(4):
            ...
            time.sleep(2 * (_k + 1))
        if not punta:
            print(f"INTEGRITA | {chain}: il nodo non dice la punta dopo 4 tentativi — "
                  f"NESSUNA finestra misurata su questa chain in questo giro", flush=True)
            continue
        punta = int(punta, 16)
        bn_now = bn_pre = None
        for _k in range(4):
            if not bn_now: bn_now, _ = rpc(url, "eth_getBlockByNumber", [hex(punta), False])
            if not bn_pre: bn_pre, _ = rpc(url, "eth_getBlockByNumber", [hex(punta - 20000), False])
            if bn_now and bn_pre: break
            time.sleep(2 * (_k + 1))
        if not bn_now or not bn_pre:
            print(f"INTEGRITA | {chain}: il nodo non dice gli istanti di riferimento dopo 4 "
                  f"tentativi — NESSUNA finestra misurata su questa chain in questo giro", flush=True)
            continue
        ...
        for b in campione:
            ...
            for pool in pool_nella_finestra:
                ...
                if ha_dentro:
                    ...
                    # UN POOL AL TETTO NON E' COMPLETO DA NESSUNA PARTE (19/09, sesta e vera
                    # versione). Le 300 righe di un pool tappato NON sono i suoi primi 300 scambi
                    # consecutivi: sono l'unione, troncata, di quello che fette diverse hanno
                    # trovato in tratti diversi. Misurato: un pool con 300 righe che coprono 48.000
                    # blocchi, cioe' 27 ore — dentro quell'intervallo i buchi ci sono PER
                    # COSTRUZIONE, e sono buchi nostri, dichiarati, non difetti.
                    # Quindi di un pool al tetto non si puo' pretendere completezza in nessun
                    # punto, e giudicarlo significa contare come guasto una regola che abbiamo
                    # scritto noi — l'errore che questo controllo ha fatto in cinque modi diversi
                    # in tre giorni.
                    # La fedelta' si misura su cio' che non e' tappato; quanto sia poco lo dice la
                    # copertura, che sta li' apposta.
                    _tot_righe = 0
                    for cart in ("storico", "vivo"):
                        f2 = f"data/multichain/{chain}/{cart}/{pool}.jsonl.gz"
                        if os.path.exists(f2):
                            try:
                                _tot_righe += sum(1 for l in gzip.open(f2, "rt") if l.strip())
                            except Exception:
                                pass
                    if _tot_righe >= int(os.environ.get("TETTO_POOL", 300)):
                        giudicabili.discard(pool)
                        al_tetto.add(pool)
                        continue
                    _tratti = []
                    for cart in ("storico", "vivo"):
