...
# LE SOGLIE NON VIVONO QUI (11/09). Stavano scritte a mano in questo file mentre i criteri di
# successo stavano in data/criteri.json: due posti, due numeri, e la possibilita' di citare quello
# comodo al momento giusto. Un traguardo che esiste in due copie e' un traguardo che si puo' scegliere
# dopo aver visto il risultato. Da qui in poi il numero e' uno solo e sta nei criteri.
def _criteri():
    try:
        return json.load(open("data/criteri.json")).get("cancello_live") or {}
    except Exception:
        return {}


_C = _criteri()
SOGLIA_ROBUSTA = float(_C.get("percentuale_robusta_minima", 40.0))
MIN_TOKEN = int(_C.get("token_minimi", 150))
MISURE_CONSECUTIVE = int(_C.get("misure_consecutive", 3))
RICHIEDE_CASSAFORTE = bool(_C.get("richiede_cassaforte", True))
