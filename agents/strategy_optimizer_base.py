#!/usr/bin/env python3
"""
STRATEGY_OPTIMIZER_BASE (loop #3) — impara la STRATEGIA di exit/timing OTTIMALE per BASE (non copiata da Robinhood:
...
data/strategy_base.json che il demo_live_base usa. Feature/path pre-calcolati per entry_h → sweep veloce. €0.
"""
...
def build(eh):
    """token Base a entry_h=eh: feature (fisse) + path (per ricalcolare il ret con vari parametri)."""
    rows = []
    for f in glob.glob(f"data/multichain/{CHAIN}/candles/*.jsonl.gz"):
        try:
            ...
            rows.append({"ent": cs[ei][0], "xt": cs[-1][0], "f": feats, "path": path})
        except: pass
...
def wf_media(rows):
    ...
    for i, r in enumerate(rows):
        ...
        if len(train) < B.WARMUP: continue
...
def main():
    ...
    if best:
        json.dump(best[1], open("data/strategy_base.json", "w"))
        ...
        with open("data/strategy_history.jsonl", "a") as fo:
            fo.write(json.dumps(dict(best[1], chain="base", ts=int(time.time()))) + "\n")
        print(f"STRATEGY_OPTIMIZER_BASE | migliore per Base: {best[1]}", flush=True)
    else:
        print("STRATEGY_OPTIMIZER_BASE | dati Base insufficienti per ottimizzare", flush=True)
