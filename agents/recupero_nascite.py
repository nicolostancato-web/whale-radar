    ...
    for banda in sorted(bande, key=lambda k: -int(k.split("-")[0])):   # dai piu' recenti: valgono di piu'
        ...
        while cur < b_a and time.time() - t0 <= BUDGET:
            ...
            for l in log:
                ...
                if t_lo is not None and t_hi is not None and fine > cur:
                    ts_ev = int(t_lo + (t_hi - t_lo) * (bn - cur) / (fine - cur))
                else:
                    ts_ev = tempo_di(per_blocco, bn)      # solo se il nodo non risponde
