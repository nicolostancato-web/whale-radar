...
def verifica(chain, contract, giorno):
    ...
    for ore in FINESTRE_ORE:
        ...
        # UNA FINESTRA SI CHIAMA T+24h SOLO SE ARRIVA A T+24h (25/09). Prendevamo l'ultima candela
        # disponibile dentro la finestra e la chiamavamo «chiusura a T+24h» anche quando i dati si
        # fermavano dopo tre ore. Il numero c'era, il nome era falso — e il nome e' cio' che uno
        # legge quando decide. Stesso difetto di `insieme.py`, `cercatore.py` e `lag_wallet.py`,
        # trovati tutti lo stesso giorno: misurare a orizzonte pieno su osservazioni corte.
        # In positivo: **accanto a ogni misura, quanto della finestra e' davvero coperto.**
        coperte = (dentro[-1][0] - fine_giorno) / 3600
        esito[f"T+{ore}h"] = {
            ...
            "volume": round(vol, 2),
            "candele": len(dentro),
            "ore_coperte": round(coperte, 1),
            "parziale": coperte < ore * 0.9,
        }
    sei = esito.get("T+6h", {})
    ...
    # E IL VERDETTO DICE SU QUANTO E' STATO PRESO (25/09). Il picco e il fondo sono presi dalla
    # finestra piu' lunga che aveva dati: se quella finestra e' coperta a meta', il verdetto resta
    # utile ma provvisorio, e deve dirlo da solo — chi lo legge fra un mese non andra' a ricontare
    # le candele.
    esito["osservazione_parziale"] = any(
        isinstance(x, dict) and x.get("parziale") for x in (sei, ventiquattro))
    if picco == 0:
