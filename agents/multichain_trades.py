...
"""
import glob, urllib.request, urllib.error, json, gzip, os, time, calendar

CHAINS = ["solana", "bsc", "base", "robinhood"]
GT = "https://api.geckoterminal.com/api/v2"
# IL BUDGET LO DECIDE CHI CHIAMA (08/09). Questa correzione era gia' stata fatta sul collector
# e non sui suoi gemelli: qui il limite restava scritto a mano e piu' lungo del tetto del
# motore, quindi il processo veniva ucciso PRIMA di salvare il segnalibro (che si scrive solo
# alla fine) e il giro dopo ripartiva dallo stesso punto. Non era lentezza: era lavoro rifatto.
# Riparare dove ho visto il problema e non dove il problema puo' stare e' come non ripararlo.
MAX_SECONDS = int(os.environ.get("BUDGET_SEC", 560))
...
def main():
    ...
    # listing (prima candela) per pool → per tenere solo la finestra iniziale (no bloat)
    firstts = {}
    for cf in glob.glob(f"{base}/candles/*.jsonl.gz"):
        a = os.path.basename(cf).replace(".jsonl.gz", "")
        try:
            ts = [int(json.loads(l)["ts"]) for l in gzip.open(cf, "rt") if json.loads(l).get("cl")]
            if ts: firstts[a] = min(ts)
        except: pass
    ...
    # PRIMA CHI E' GIA' VALUTABILE (14/09). Questo collettore e quello delle candele camminavano sulla
    # stessa lista da 18.700 pool partendo da estremi diversi, e non si incontravano mai: su Robinhood
    # abbiamo 4.281 file di scambi e ZERO in comune coi 400 pool che sappiamo valutare. Non sono dati
    # sbagliati: sono dati che non incontrano mai gli altri, e quindi non possono diventare una
    # variabile. Un archivio che non si unisce a niente e' peso, non ricchezza.
    # Adesso, a parita' di tutto, vengono prima i pool che HANNO GIA' una serie di prezzo: ogni file
    # scaricato li' diventa subito una riga interrogabile su chi compra, in che ordine, con che tempi.
    valutabili = set()
    for d in ("candles", "pulse"):
        for f in glob.glob(f"{base}/{d}/*.jsonl.gz"):
            valutabili.add(os.path.basename(f).replace(".jsonl.gz", ""))
    if valutabili:
        gia = [a for a in todo if a in valutabili]
        altri = [a for a in todo if a not in valutabili]
        manca = [a for a in (never + young + stale) if a in valutabili and a not in todo]
        # meta' del giro ai pool gia' valutabili non ancora coperti: il raccordo si chiude da solo
        quota = max(1, TRADE_BATCH // 2)
        todo = (gia + manca[:quota] + altri)[:TRADE_BATCH]
    ...
    for addr in todo:
        ...
        for t in d["data"]:
            ...
            seen.add(tx)
            # QUANDO L'ABBIAMO SAPUTO NOI (14/09). Senza questo timbro non si puo' rispondere alla
            # domanda che decide la validita' di ogni ricerca: "questo dato ce l'avevamo, al momento
            # in cui la strategia avrebbe deciso?". Provare a ricostruirlo a posteriori dalla storia
            # git non funziona: il nostro stesso GC ha schiacciato la storia l'11/09 e ha cancellato
            # ogni traccia di acquisizione precedente. Da oggi il momento in cui il dato entra da noi
            # viaggia DENTRO il dato, dove nessuna manutenzione puo' cancellarlo.
            rows.append({"acq": nowi, "tx": tx, "ts": to_epoch(a.get("block_timestamp", "")), "kind": a.get("kind"),
                         "usd": float(a.get("volume_in_usd") or 0), "w": a.get("tx_from_address")})
            added += 1
        if added:
            if addr in firstts:                             # tieni solo listing → +4h (feature identiche, no bloat)
                cut = firstts[addr] + 4 * 3600
    ...
    try:
        import registro_pit as _R
        _R.annota("multichain_trades", "geckoterminal", chain, total,
                  {"pool_interrogati": len(todo)})
    except Exception:
        pass
