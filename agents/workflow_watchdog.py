...
def main():
    ...
    for w in active:
        ...
        if atteso:
            # PER I LAVORI RARI IL MOLTIPLICATORE E' SBAGLIATO (20/09). Con «due volte e mezzo il
            # periodo dichiarato», un lavoro giornaliero viene considerato fermo solo dopo SESSANTA
            # ore. Misurato oggi: la compattazione del repo, pianificata alle 03:00, non e' partita
            # (cron saltato, come succede quando il repo e' occupato) e il guardiano l'avrebbe
            # ignorata fino a dopodomani — mentre la cronologia cresceva di cento megabyte l'ora.
            # Per i lavori frequenti il margine proporzionale va bene; per quelli radi serve un
            # margine FISSO: sei ore di ritardo su un giornaliero sono gia' troppe.
            stale = max(STALE_HOURS, atteso + 6 if atteso >= 12 else atteso * 2.5)
        else:
