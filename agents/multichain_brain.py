...
def load_rows(chain):
    ...
    for f in serie_files(chain):
        try:
            ...
            # L'INDIRIZZO VIENE CON NOI (11/09). Finora una riga diceva quanto aveva reso ma non SU
            # COSA: impossibile tornare alle sue candele e chiedersi se il movimento fosse gia'
            # successo prima della nostra entrata. Un risultato senza il suo soggetto non si puo'
            # interrogare due volte. Campo in piu', nessun comportamento cambiato.
...
def walkforward_righe(rows, thr=THR):
    """Come walkforward, ma restituisce le RIGHE scelte invece dei soli rendimenti: stessa identica
    logica, cosi' non esistono due selezioni che possono divergere in silenzio."""
    sel = []; model = None; last_n = 0
    for i, r in enumerate(rows):
        train = [q for q in rows[:i] if q["xt"] < r["ent"]]
        if len(train) < WARMUP: sel.append(r); continue
        ...
        if model is None: sel.append(r); continue
        w, b, mu, sd = model
        s = L.sigmoid(sum(w[j] * (r["f"][j] - mu[j]) / sd[j] for j in range(len(r["f"]))) + b)
        if s >= thr: sel.append(r)
    return sel


def walkforward(rows, thr=THR):
    return [r["ret"] for r in walkforward_righe(rows, thr)]
