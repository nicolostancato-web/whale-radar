...
def rendimento(cs, quando, frontiera=None):
    """L'esito a ORIZZONTE. Distingue DUE motivi diversi per cui la candela finale puo' mancare.

    IL 25/09 HO SBAGLIATO QUESTA FUNZIONE DUE VOLTE IN UN GIORNO, in direzioni opposte.
    - Com'era: se mancava il prezzo a 24 ore prendeva l'ultimo disponibile e lo chiamava «esito a
      24 ore». Per un wallet entrato tre ore fa, un rendimento a tre ore spacciato per uno a
      ventiquattro — e poiche' i token nuovi salgono nelle prime ore e scendono dopo, l'errore
      gonfiava proprio i wallet piu' recenti, quelli che stiamo valutando adesso.
    - Come l'avevo «corretta» la mattina: niente candela finale, niente esito. Peggio. Un token
      muore smettendo di scambiare: la candela manca PROPRIO PERCHE' e' andato a zero. Quella
      versione scartava i fallimenti e teneva i sopravvissuti. Misurato altrove lo stesso giorno,
      il criterio «ore osservate» abbassava le trappole di sette punti (42,6% vero contro 35,2%).

    IN POSITIVO: **la candela che manca vuol dire due cose diverse, e la frontiera della raccolta
    dice quale.** Se la raccolta e' arrivata oltre l'orizzonte e la candela non c'e', il token e'
    morto: l'ultimo prezzo E' l'esito. Se la raccolta stessa non e' arrivata fin la', non c'e'
    ancora un esito: None, e si aspetta."""
    e = prezzo_a(cs, quando)
    if not e or e <= 0: return None
    u = prezzo_a(cs, quando + ORIZZONTE)
    if u:
        return u / e - 1
    if frontiera is not None and frontiera >= quando + ORIZZONTE:
        return (cs[-1][4] / e - 1) if cs[-1][4] else None     # morto: l'ultimo prezzo e' l'esito
    return None                                               # troppo presto: non e' un esito
...
def main():
    ...
    # LA FRONTIERA DELLA RACCOLTA: l'ultima candela vista su tutta la chain. Serve a distinguere
    # «il token e' morto» da «non abbiamo ancora guardato fin la'» (vedi `rendimento`).
    frontiera = max((cs[-1][0] for cs in serie.values()), default=0)
