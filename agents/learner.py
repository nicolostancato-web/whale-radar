...
MINIMI = {}      # pool -> {ts: minimo della candela}, riempito da load_data()
    ...
    for f in glob.glob("data/raw/candles/*.jsonl.gz"):
        try:
            for l in gzip.open(f, "rt"):
                d = json.loads(l)
                if d["tf"] == "hour" and d.get("cl"):
                    cand.setdefault(d["pool"], {})[int(d["ts"])] = d["cl"]
                    # il MINIMO della candela c'era nei dati e non lo leggevamo: serve al rug intrabar
                    # (dentro l'ora il prezzo puo' crollare e risalire, e lo stop va valutato sul fondo)
                    MINIMI.setdefault(d["pool"], {})[int(d["ts"])] = d.get("l") or d["cl"]
        except: pass
...
def _ritardo_osservazione():
    """Quanto tardi i dati arrivano DA NOI, misurato (vedi agents/disponibilita.py).

    Aggiunto il 04/09 dopo la revisione esterna: *"aggiungere il ritardo reale del bot"*.
    Le feature erano gia' oneste sul passato — verificato ricalcolandole senza i dati futuri, e
    vengono identiche. Ma essere oneste non basta: un bot vero non puo' decidere su una candela che
    non ha ancora scaricato, e la nostra catena ha un ritardo misurato di 3-7 ore.
    Senza questo taglio entriamo all'ora giusta con informazioni che a quell'ora non avevamo:
    non e' la stessa strategia con un handicap, e' una strategia diversa.

    RITARDO=0 lo disattiva, per poter confrontare i due mondi e vedere quanto pesa davvero.
    Nel dubbio si usa il ritardo PEGGIORE fra le chain: sbagliare per prudenza si paga in occasioni
    perse, sbagliare per ottimismo si paga in soldi."""
    import os as _os
    forz = _os.environ.get("RITARDO")
    if forz is not None:
        try: return max(0, int(float(forz)))
...
RITARDO_OSS = _ritardo_osservazione()
...
def features_at_entry(pool, entry_ts, cand, flow, fb_pool, wallet_listings, first_ts):
    ...
    E arrivate davvero fino a noi (vedi RITARDO_OSS)."""
    visto_fino_a = entry_ts - RITARDO_OSS
    fl = flow.get(pool, {}); cs = cand.get(pool, {})
    past = sorted([(h, b, s) for h, (b, s) in fl.items() if h <= visto_fino_a])
    ...
    ks = sorted(cs)
    _vis = [k for k in ks if k <= visto_fino_a]
    dump = cs[_vis[-1]] / cs[ks[0]] if _vis and ks and cs[ks[0]] else 1.0
    ...
    for w in wallets:
        if sum(1 for l in wallet_listings.get(w, []) if l < min(lt, visto_fino_a)) >= 2: smart += 1
...
# I COSTI NON SI ASSUMONO PIU': arrivano da metro.py, che li legge dalle misure vere su Jupiter
# (decisione investitore 03/09, vedi DECISIONS.md). METRO=stress rimette il vecchio 33% come
# scenario pessimistico. Prima erano scritti qui a mano: 15% per lato, mai misurato, e con quel
# numero abbiamo dichiarato morte quattro chain.
import sys, os as _os
sys.path.insert(0, _os.path.dirname(_os.path.abspath(__file__)))
import metro as _M
ES = _M.ES; XS = _M.XS; FEE = _M.FEE; GAS = _M.GAS; SIZE = _M.SIZE; LAT = _M.LAT
L1, L2 = 3.0, 6.0; TRAIL = 0.50; HARD = 0.70
...
def _net(m, tr=False, fuga=None):
    """rendimento NETTO di una gamba. Costi reali, non ottimistici (rivisto 31/08 dopo la revisione critica).

    Cosa e' cambiato: la latenza veniva applicata SOLO alle uscite in trailing. Ma anche un'uscita a target
    arriva in ritardo — l'ordine parte quando il prezzo ha gia' toccato il livello, e su un memecoin in quei
    secondi il prezzo si e' mosso. Ora la latenza pesa su OGNI uscita, doppia su quelle in trailing (li' stai
    inseguendo un prezzo che sta gia' scendendo)."""
    # `fuga`: stai uscendo perche' il prezzo ti sta scappando (stop/trailing), non perche' hai
    # raggiunto il tuo obiettivo. Aggiunto il 03/09 dopo l'audit: usavamo lo stesso slippage nei due
    # casi, ma sono due mercati diversi — lo stop scatta quando tutti vendono e il pool si prosciuga.
    # Default: se e' un'uscita in trailing, e' una fuga.
    ein = (1 + ES) * (1 + FEE)
    ritardo = LAT * (2 if tr else 1)
    eout = m * (1 - _M.uscita(tr if fuga is None else fuga)) * (1 - FEE) * (1 - ritardo)
