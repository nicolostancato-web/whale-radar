...
def main():
    ...
    for k, (res, plac, n) in esiti.items():
        ...
        if momentum:
            # LA SECONDA DOMANDA (06/09, dalla revisione esterna). "C'era momentum" NON basta a
            # bocciare: un segnale puo' descrivere il passato E prevedere il futuro, se il fenomeno
            # persiste. La domanda giusta e': *a parita' di rialzo gia' avvenuto*, aggiunge qualcosa?
            # Prima chiudevo qui e passavo oltre — cosi' un candidato vivo sarebbe morto senza appello.
            pari = {h: [] for h in C.ORIZZONTI}
            for ts, addr, _ in alti:
                key = ts // 3600
                if key not in vivi_cache: vivi_cache[key] = U.vivi(ts)
                ctrl2 = [U.cs[c] for c in C.matched_su_momentum(U, addr, ts, k=5,
                                                                candidati=vivi_cache[key])]
                if not ctrl2: continue
                for h, x in C.excess(U.cs[addr], ctrl2, ts).items(): pari[h].append(x)
            righe2 = []
            for h in C.ORIZZONTI:
                v_ = pari[h]
                if len(v_) < 15: continue
                righe2.append(f"| {C.ETICHETTE[h]} | **{st.mean(v_)*100:+.1f}%** | "
                              f"{st.median(v_)*100:+.1f}% | {len(v_)} |")
            L += ["", f"> ⚠️ Prima del segnale il token aveva già fatto {ind*100:+.0f}%, dopo fa "
                      f"{av*100:+.1f}%. **Ma questo non basta a bocciarlo**: la domanda vera è se,",
                  "> **a parità di rialzo già avvenuto**, il segnale aggiunga qualcosa.", ""]
            if righe2:
                L += ["### Confronto con token che avevano fatto lo STESSO rialzo", "",
                      "| dopo | extra-rendimento | mediano | casi |", "|---|---|---|---|"] + righe2
                pos2 = [h for h in C.ORIZZONTI if len(pari[h]) >= 15
                        and st.mean(pari[h]) > 0 and st.median(pari[h]) > 0]
                if len(pos2) >= 2:
                    L += ["", "> ✅ **Sopravvive al confronto giusto**: a parità di momentum precedente",
                          "> il segnale aggiunge valore. Il vecchio criterio l'avrebbe ucciso per sbaglio.", ""]
                    passati.append(k)
                else:
                    L += ["", "> ❌ A parità di rialzo precedente non aggiunge niente: era davvero solo",
                          "> il ricordo di un movimento altrui.", ""]
            else:
                L += ["> *Non ci sono abbastanza token con lo stesso rialzo per il confronto: sospeso.*", ""]
