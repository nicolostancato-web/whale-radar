...
*2026-09-12 05:54 UTC · un meeting per goal, a ogni ciclo (~30 min)*

## Architetto: 🟢 tutti i loop si stanno riunendo

| loop | la domanda | dove siamo | ago | cosa si fa |
|---|---|---|---|---|
| `accumulo-base` | Stiamo raccogliendo token Base nuovi? | 4242 token con dati (2903 osservati dal vivo, 1339 con storico) su 14629 pool noti | ⏸ fermo dall'ultimo meeting | ✋ mano alzata: tutti i rimedi noti hanno gia' fallito 3 volte (pulse.py, multichain_collector.py) → non li ripetiamo, se |
| `accumulo-solana` | Stiamo raccogliendo token Solana nuovi? | 1239 token con dati (176 osservati dal vivo, 1063 con storico) su 14650 pool noti | ⏸ fermo da 5h | 🚨 fermo da 5h e la scala e' finita: serve una decisione umana |
...
| `accumulo-robinhood` | Stiamo raccogliendo token Robinhood nuovi? | 2398 token con dati (116 osservati dal vivo, 2282 con storico) su 18961 pool noti | 📈 si muove | avanti cosi' |
| `accumulo-bsc` | Stiamo raccogliendo token BSC nuovi? | 1027 token con dati (246 osservati dal vivo, 781 con storico) su 17417 pool noti | ⏸ fermo da 6h | 🚨 fermo da 6h e la scala e' finita: serve una decisione umana |
| `percentuale-bsc` | Come alziamo la percentuale su BSC? | robusta -24% (media -21%) su 600 token | ⏸ fermo da 19h | ✋ mano alzata: tutti i rimedi noti hanno gia' fallito 3 volte (explorer.py, learner.py) → non li ripetiamo, serve cambia |

## 🎯 Le 3 cose che contano adesso

1. **percentuale-bsc** — fermo da 19h. robusta -24% (media -21%) su 600 token
