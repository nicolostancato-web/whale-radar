...
*2026-09-15 23:56 UTC · un meeting per goal, a ogni ciclo (~30 min)*

## Architetto: 🟢 tutti i loop si stanno riunendo

| loop | la domanda | dove siamo | ago | cosa si fa |
|---|---|---|---|---|
| `accumulo-base` | Stiamo raccogliendo token Base nuovi? | 4848 token con dati (3318 osservati dal vivo, 1530 con storico) su 14838 pool noti | 📈 si muove | avanti cosi' |
...
| `percentuale-base` | Come alziamo la percentuale su Base? | robusta -8% (media -3%) su 1909 token | ⏸ fermo da 12h | ✋ mano alzata: tutti i rimedi noti hanno gia' fallito 3 volte (strategy_optimizer_base.py, learner.py) → non li ripetiam |
| `percentuale-solana` | Come alziamo la percentuale su Solana? | robusta -26% (media -24%) su 769 token | ⏸ fermo da 1h | ✋ mano alzata: tutti i rimedi noti hanno gia' fallito 3 volte (strategy_optimizer_solana.py, wallet_insider.py) → non li |
...
| `accumulo-robinhood` | Stiamo raccogliendo token Robinhood nuovi? | 2575 token con dati (233 osservati dal vivo, 2342 con storico) su 19060 pool noti | 📈 si muove | avanti cosi' |
| `accumulo-bsc` | Stiamo raccogliendo token BSC nuovi? | 1074 token con dati (300 osservati dal vivo, 774 con storico) su 17481 pool noti | ⏸ fermo dall'ultimo meeting | ✋ mano alzata: tutti i rimedi noti hanno gia' fallito 3 volte (pulse.py, multichain_collector.py, multichain_rpc.py) → n |
| `percentuale-bsc` | Come alziamo la percentuale su BSC? | robusta -24% (media -21%) su 604 token | ⏸ fermo da 9h | ✋ mano alzata: tutti i rimedi noti hanno gia' fallito 3 volte (explorer.py, learner.py) → non li ripetiamo, serve cambia |

## 🎯 Le 3 cose che contano adesso

1. **percentuale-base** — fermo da 12h. robusta -8% (media -3%) su 1909 token
2. **percentuale-robinhood** — fermo da 10h. robusta -26% (media -17%) su 45 token
3. **percentuale-bsc** — fermo da 9h. robusta -24% (media -21%) su 604 token
