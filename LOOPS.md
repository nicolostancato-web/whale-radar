...
*2026-09-05 09:02 UTC · un meeting per goal, a ogni ciclo (~30 min)*

## Architetto: 🟢 tutti i loop si stanno riunendo

| loop | la domanda | dove siamo | ago | cosa si fa |
|---|---|---|---|---|
| `accumulo-base` | Stiamo raccogliendo token Base nuovi? | 4587 token con dati (2233 osservati dal vivo, 2354 con storico) su 15737 pool noti | 📈 si muove | avanti cosi' |
| `accumulo-solana` | Stiamo raccogliendo token Solana nuovi? | 1230 token con dati (129 osservati dal vivo, 1101 con storico) su 14655 pool noti | ⏸ fermo da 1h | ✋ mano alzata: tutti i rimedi noti hanno gia' fallito 3 volte (solana_helius.py, multichain_collector.py) → non li ripet |
...
| `percentuale-solana` | Come alziamo la percentuale su Solana? | robusta -13% (media -9%) su 702 token | ⏸ fermo da 11h | ✋ mano alzata: tutti i rimedi noti hanno gia' fallito 3 volte (strategy_optimizer_solana.py, wallet_insider.py) → non li |
...
| `accumulo-robinhood` | Stiamo raccogliendo token Robinhood nuovi? | 817 token con dati (0 osservati dal vivo, 817 con storico) su 17282 pool noti | ⏸ fermo dall'ultimo meeting | ✋ mano alzata: tutti i rimedi noti hanno gia' fallito 3 volte (whale_candles.py, whale_backfill.py, multichain_collector |
| `accumulo-bsc` | Stiamo raccogliendo token BSC nuovi? | 1159 token con dati (146 osservati dal vivo, 1013 con storico) su 17485 pool noti | ⏸ fermo dall'ultimo meeting | ✋ mano alzata: tutti i rimedi noti hanno gia' fallito 3 volte (pulse.py, multichain_collector.py, multichain_rpc.py) → n |
...
2. **percentuale-solana** — fermo da 11h. robusta -13% (media -9%) su 702 token
