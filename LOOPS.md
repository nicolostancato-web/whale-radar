...
*2026-09-05 19:46 UTC · un meeting per goal, a ogni ciclo (~30 min)*

## Architetto: 🟢 tutti i loop si stanno riunendo

| loop | la domanda | dove siamo | ago | cosa si fa |
|---|---|---|---|---|
| `accumulo-base` | Stiamo raccogliendo token Base nuovi? | 4976 token con dati (2342 osservati dal vivo, 2634 con storico) su 16019 pool noti | 📈 si muove | avanti cosi' |
| `accumulo-solana` | Stiamo raccogliendo token Solana nuovi? | 1313 token con dati (134 osservati dal vivo, 1179 con storico) su 14773 pool noti | 📈 si muove | avanti cosi' |
...
| `accumulo-robinhood` | Stiamo raccogliendo token Robinhood nuovi? | 909 token con dati (0 osservati dal vivo, 909 con storico) su 17402 pool noti | 📈 si muove | avanti cosi' |
| `accumulo-bsc` | Stiamo raccogliendo token BSC nuovi? | 1215 token con dati (149 osservati dal vivo, 1066 con storico) su 17565 pool noti | 📈 si muove | avanti cosi' |
| `percentuale-bsc` | Come alziamo la percentuale su BSC? | robusta -24% (media -21%) su 583 token | ⏸ fermo da 29h | ✋ mano alzata: tutti i rimedi noti hanno gia' fallito 3 volte (explorer.py, learner.py) → non li ripetiamo, serve cambia |

## 🎯 Le 3 cose che contano adesso

1. **percentuale-bsc** — fermo da 29h. robusta -24% (media -21%) su 583 token
