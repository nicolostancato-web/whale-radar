...
*2026-09-11 22:07 UTC · un meeting per goal, a ogni ciclo (~30 min)*

## Architetto: 🟢 tutti i loop si stanno riunendo

| loop | la domanda | dove siamo | ago | cosa si fa |
|---|---|---|---|---|
| `accumulo-base` | Stiamo raccogliendo token Base nuovi? | 4179 token con dati (2878 osservati dal vivo, 1301 con storico) su 14609 pool noti | ⏸ fermo da 2h | 🚨 fermo da 2h e la scala e' finita: serve una decisione umana |
...
| `percentuale-solana` | Come alziamo la percentuale su Solana? | robusta -17% (media -12%) su 761 token | ⏸ fermo da 6h | ✋ mano alzata: tutti i rimedi noti hanno gia' fallito 3 volte (strategy_optimizer_solana.py, wallet_insider.py) → non li |
...
| `accumulo-robinhood` | Stiamo raccogliendo token Robinhood nuovi? | 2180 token con dati (101 osservati dal vivo, 2079 con storico) su 18701 pool noti | 📈 si muove | avanti cosi' |
| `accumulo-bsc` | Stiamo raccogliendo token BSC nuovi? | 1027 token con dati (246 osservati dal vivo, 781 con storico) su 17417 pool noti | 📈 si muove | avanti cosi' |
...
2. **percentuale-solana** — fermo da 6h. robusta -17% (media -12%) su 761 token
